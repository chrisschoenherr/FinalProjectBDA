...
**Your Answers here:**
<br>
1. First, in the file dataSet1.R we define the tickers we want to download in row 8: mytickers = c("SPY", "AAPL"), then, there's a for-loop (row 20) that runs through all the values (in this case just SPY and AAPL) and downloads them.
<br>

1. The cumulative returns of the AAPL stock during this period is `r round(100*sum(StockReturns[,2]),1)`%.
2. The average daily returns of the AAPL stock during this period is `r round(100*mean(StockReturns[,2]),3)`%;
2. The standard deviation of the AAPL stock of the S&P index during this period is `r round(100*sd(StockReturns[,2]),3)`%;
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
...
<br>
<br>
1. The cumulative returns of the YHOO stock during this period is `r round(100*sum(StockReturns[,3]),1)`%.
2. The average daily returns of the YHOO stock during this period is `r round(100*mean(StockReturns[,3]),3)`%;
2. The standard deviation of the YHOO stock of the S&P index during this period is `r round(100*sd(StockReturns[,3]),3)`%;
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
...
**Your Answers here:**
<br>
Answer 1 <br>: In order to get the number of rows and number of columns we can use the function dim(transposedData). Dim stands for dimension. The output will be "3 3874", meaning there's 3 rows and 3874 columns. Alternatively we can use nrow(transposedData) and ncol(transposedData)
<br>
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposedData = t(StockReturns)
portfolioT = apply(transposedData,2,mean)
names(portfolioT) <- colnames(transposedData)
pnl_plot(portfolioT)
```
<br>
...
<br>
<br>
Answer 1: In dataSet1.R, Row 9, there's the variable "startDate", which is all we have to change to "2001-01-01"
<br>
<br>
<hr>
<hr>
...
**Your Answers here:**
<br>
Question 1: This is basically a way to count the number of rows in the cvs. 
<br>
<br>
Question 2: I'm getting the error "Error in FUN(left, right) : non-numeric argument to binary operator". I'm assuming this is because the data in the matrix myData and in StockReturns have different formats (e.g. the first column in myData is a numeric value, but in StockReturns it's in the "date" format)
<br>
